...
export class Renderer {
  ...
  render(timeline, t, tempoManager, audioManager, fps) {
    ...
    /** @type {import('./factories.js').TimelineComputeItem[]} */
    const computes = /** @type {unknown} */ (
    ...
    for (const comp of computes) {
      ...
      /** @type {unknown} */ (comp.logic)(t, defaultBeat, spectrum, w, h);
    }

    // Visual要素の抽出とZ-index安定ソート
    /** @type {(import('./factories.js').TimelineVisualItem<unknown> & { _originalIndex: number })[]} */
    const visuals = /** @type {unknown} */ (
      timeline
    ...
    // グローバルエフェクトの描画
    /** @type {import('./factories.js').TimelineGlobalEffectItem[]} */
    const globalEffects = /** @type {unknown} */ (
