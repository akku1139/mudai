...
/**
 * プロパティの値を評価します。関数であれば実行し、文字列の座標指定であれば計算します。
 * @template T
 * @param {T | ((t: number, b: number) => T)} param 評価するプロパティ値または関数
 * @param {number} t 現在の時間(秒)
 * @param {number} b 現在のビート
 * @param {number} [refSize] キャンバスの基準サイズ(幅や高さなど)
 * @returns {Exclude<T, string> | number} 文字列式が渡された場合は計算されてnumberになります
 */
export function evaluateProp(param, t, b, refSize = undefined) {
  ...
  return /** @type {Exclude<T, string> | number} */ (val);
}
