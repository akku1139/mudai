 ...
 * @template {Record<string, unknown>} TProps コンポーネント固有のプロパティの型
 * @template {Record<string, unknown>} TCustomState コンポーネント固有の状態の型
 * @param {RenderLogic<TProps, TCustomState>} renderLogic 実際の描画を行う関数
 * @returns {(props: BaseProps & TProps) => (ctx: CanvasRenderingContext2D | OffscreenCanvasRenderingContext2D, t: number, b: number, s: unknown, vw: number, vh: number) => void}
 */
export function createComponent(renderLogic) {
  return (/** @type {BaseProps & TProps} */ props) => {
    ...
    return (/** @type {CanvasRenderingContext2D | OffscreenCanvasRenderingContext2D} */ ctx, /** @type {number} */ t, /** @type {number} */ b, /** @type {unknown} */ s, /** @type {number} */ vw, /** @type {number} */ vh) => {
      // シークやリセットを検知して状態を初期化
