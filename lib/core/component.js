...
/**
 * 重複チェック用ユーティリティ
 * T と B に共通のキーがあれば never、なければ T を返す
 * @template T
 * @template B
 * @typedef {keyof T & keyof B extends never ? T : never} ValidateNoOverlap
 */
...
/**
 * @template {object} [TProps={}] コンポーネント固有のプロパティ
 * @template {BaseProps} [B=BaseProps]
 * @typedef {ValidateNoOverlap<TProps, B> extends never
 * ? "Error: TProps contains keys that overlap with BaseProps" // 重複がある場合
 * : B & { [K in keyof TProps]: TProps[K] } & { effects?: Effect[] }
 * } RenderProps
 */

// {{[K in keyof B]: DynamicProp<B[K]>} & {[K in keyof T]: K extends keyof B ? never : DynamicProp<T[K]>} & {effects?: Effect[]} }
 ...
 * @returns {(props: RenderProps<TProps>) => (ctx: CanvasRenderingContext2D | OffscreenCanvasRenderingContext2D, t: number, b: number, s: Uint8Array, vw: number, vh: number) => void}
 */
export function createComponent(renderLogic) {
  return (/** @type {RenderProps<TProps>} */ props) => {
