...
import { visual, globalEffect, compute } from './core/factories.js';
import { createState } from './core/state.js';
import { animate } from './core/animation.js';

/**
 * @type {import('./core/engine.js').EngineConfig}
 */
export const config = {
  width: 1920,
  height: 1080,
...
export const timeline = [
  ...
  */
  ...
  compute((t, b) => {
    ...
    sharedColor.current = /** @type {string} */(colors[Math.floor(b) % colors.length]);
  ...
  visual(textObject, {
    ...
    size: 80,
    rotation: (t) => t * 45,
    scale: (_t, b) => 1 + Math.sin(b * Math.PI) * 0.2
  }),
